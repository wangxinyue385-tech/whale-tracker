...
def request_json(path: str, params: dict[str, Any] | None = None, timeout: float = 8) -> Any:
    errors = []
    hosts = [BINANCE_REST.rstrip("/")] + [x for x in BINANCE_REST_FALLBACKS if x != BINANCE_REST.rstrip("/")]
    for host in hosts:
        try:
            response = HTTP.get(host + path, params=params, timeout=(3, timeout))
            response.raise_for_status()
            return response.json()
        except Exception as exc:
            errors.append(f"{host}: {exc}")
    raise RuntimeError("Binance REST unavailable; " + " | ".join(errors[-3:]))
...
def debug_binance() -> Any:
    ...
    for host in hosts:
        started = time.time()
        try:
            response = HTTP.get(host + "/fapi/v1/time", timeout=(3, 5))
